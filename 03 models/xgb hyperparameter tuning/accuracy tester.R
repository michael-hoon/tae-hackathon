      ...
      subset1_train <- subset(df_train, select = -c(Task , educ, gender, region, segment, ppark, night, miles, Case, No, CC4, GN4, NS4, BU4, FA4, LD4, BZ4, FC4, FP4, RP4, PP4, KA4, SC4, TS4, NV4, MA4, LB4, AF4, HU4, Price4, Urb, income, age))
      subset1_test <- subset(df_test, select = -c(Task, educ, gender, region, segment, ppark, night, miles, Case, No, CC4, GN4, NS4, BU4, FA4, LD4, BZ4, FC4, FP4, RP4, PP4, KA4, SC4, TS4, NV4, MA4, LB4, AF4, HU4, Price4, Urb, income, age))
      ...
      for(p in 12:12){
        ...
        for (i in 1:k) {
          ...
          X_train <- train_data %>% select(-c(Choice,Ch1,Ch2,Ch3,Ch4))
          X_val <- val_data %>% select(-c(Choice,Ch1,Ch2,Ch3,Ch4))
      ...
      xgb_preds <- predict(xgb_model, as.matrix(X_val), reshape = TRUE)
      ...
      compute_accuracy(xgb_preds,(val_data[,77:80]))