...
#HYPERPARAMETER TUNING - TRYING TO FIND ETA
traintask <- makeClassifTask (data = train,target = "Choice")
testtask <- makeClassifTask (data = test,target = "Choice")

xgb_learn <- makeLearner("classif.xgboost",predict.type = "response",par.vals = list(objective="multi:softprob", eval_metric="mlogloss", nrounds=1000))
?makeLearner
xgb_hyptun <- makeParamSet(makeDiscreteParam("booster",values = "gbtree"),
  makeNumericParam("eta", lower = 0.01, upper = 0.05)
)


#for next time
# xgb_hyptun <- makeParamSet(
...
resample <- makeResampleDesc("CV", iters = 4)
...
subset_test <- subset(df_test,select=-c(Task,Ch1,Ch2,Ch3,Ch4,educ,gender,region,segment,ppark,night,miles,Case,CC4,GN4,NS4,BU4,FA4,LD4,BZ4,FC4,FP4,RP4,PP4,KA4,SC4,TS4,NV4,MA4,LB4,AF4,HU4,Price4,Urb,income,age, KA3, HU2, GN3,
                                        FC2, KA2, HU3, FA3, FC3, FA2, RP3, RP2))
...
xgb_preds
write.csv(xgb_preds, "0728_submission3_xgb_hypparaiter1.csv")
