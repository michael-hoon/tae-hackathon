...
# best eta = 0.06, nrounds = 125. - take 1 up and 1 down for sensitivity.
# eta = 0.05 - 0.07, step = 0.1
# nrounds = 100 - 150, step = 25
# Trying to find best max depth and subsample
# max_depth in range 3 - 8, step = 1
# subsamples in range 0.5 - 1, step = 0.25
...
for (a in seq(100,150, by=25)) {
  for (b in 5:7) {
    for(c in 3:8) {
      for (d in seq(50,100, by=25)) {
        xgb_params <- list(
          booster = "gbtree",
          eta = (b/100),
          max_depth = c,
