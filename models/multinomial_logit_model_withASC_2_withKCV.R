...
for (i in 1:k) {
  ...
  M_ASC <- mlogit(Choice~GN+NS+BU+FA+LD+BZ+FC+FP+RP+PP+KA+SC+TS+NV+MA+LB+AF+HU+Price-1 
                  | yearind+milesind+pparkind+genderind+ageind+Urbind+incomeind, 
                  data=S, heterosc=TRUE)
  summary(M_ASC)
...
# P_test <- predict(M_ASC, newdata=S_test)
# 
